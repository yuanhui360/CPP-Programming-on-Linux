...
void amqp_usage()
{
    std::cout << "Usage: amqp_publish <host:port> <exchange:type> <rounting key|topic pattern> [queue]" << std::endl;
    std::cout << "Example :" << std::endl;
    std::cout << "amqp_publish 192.168.0.103:5672 ExchangeName:direct rt_key direct_queue" << std::endl;
    std::cout << "amqp_publish 192.168.0.103:5672 ExchangeName:topic topic|pattern topic_queue" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    res = amqp_login(conn, "my_vhost", 0, AMQP_DEFAULT_FRAME_SIZE, 0, AMQP_SASL_METHOD_PLAIN, "test", "test");
    ...
    if ( eVec[1] == "headers" ) {
        ...
        std::cout << "Loop Header (Key:Value) Start: x-match:any or x-match:all, Finish loop: end:end" << std::endl;
        while (1) {
            hdrKeyValStr.clear();
            hVec.clear();
    ...
    amqp_exchange_declare(conn, my_channel, ex_name, ex_type,
    ...
    if ( qu_name.len > 0) {
         ...
         *   ex_args.num_entries = 0;
         *   ex_args.entries = NULL;
          */
        ...
        res = amqp_simple_rpc(conn, my_channel, AMQP_QUEUE_DECLARE_METHOD, &method_ok , &s);
        amqp_release_buffers(conn);
        amqp_queue_bind(conn, my_channel, qu_name, ex_name, rt_key, qu_args);
    }
    ...
    while (1) {
        ...
        amqp_basic_publish(conn, my_channel, ex_name, rt_key, mandatory, immediate,
