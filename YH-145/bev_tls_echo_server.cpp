                      ...
                      struct sockaddr *sock_addr, int sock_len, void *param)
{
    ...
    SSL_CTX *ctx = (SSL_CTX *) param;
    struct event_base *base = evconnlistener_get_base(listener);
    struct bufferevent *bev = bufferevent_openssl_socket_new(base, fd, SSL_new(ctx),
...
int main(int argc, char *argv[])
{
    ...
    int rc;
    if ( argc != 4 ) {
        printf("usage: %s <port> <certificate> <private key>\n", argv[0]);
        printf("Example : %s 1234 my_certificate.pem my_private_key.key\n", argv[0]);
        exit(-1);
    }
    ...
    if ( ctx == nullptr ) {
        printf("Unable to create SS: context\n");
        exit(-1);
    }
    ...
    struct evconnlistener *listener = evconnlistener_new_bind(base, accept_cb, ctx,
