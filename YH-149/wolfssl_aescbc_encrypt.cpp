...
int main(int argc, char **argv)
{
    ...
    if ( argc != 3 ) {
        printf("usage: %s <text file> <encypted file>\n", argv[0]);
        printf("Example : %s my_message.txt my_encrypted.dat\n", argv[0]);
        ret = -1;
        return ret;
    }
    ...
    XFILE text_fp = XFOPEN(argv[1], "rb");
    XFILE cipher_fp = XFOPEN(argv[2], "wb");
    assert( text_fp || cipher_fp );

    word32 n_read = (word32)XFREAD(text_buf, 1, BLOCK_BUFFER_SIZE, text_fp);
    if ( n_read <= 0 ) {
    ...
    wc_AesInit(&enc, NULL, INVALID_DEVID);      // <-- Initialize Aes structure.
        ...
        //   ... TODO ...
        ret = wc_AesSetKey(&enc, key, AES_BLOCK_SIZE, iv, AES_ENCRYPTION);  // <-- Set AES struct Key and
        ret = wc_AesCbcEncrypt(&enc, cipher_buf, text_buf, BLOCK_BUFFER_SIZE);
        XFWRITE(cipher_buf, 1, BLOCK_BUFFER_SIZE, cipher_fp);
