...
struct global_data
{
    ...
    char        Server[256];
    int         Port;
    char        proxy_to_host[256];
    int         proxy_to_port;
    char        Login[256];
    char        PassWd[256];
    char        rsa_key[256];
    global_data() : id(0), Port(0) , proxy_to_port(0)
    {
        memset(Server,'\0', sizeof(Server));
        memset(Login, '\0', sizeof(Login));
        memset(PassWd, '\0', sizeof(PassWd));
        memset(rsa_key, '\0', sizeof(rsa_key));
        memset(proxy_to_host, '\0', sizeof(proxy_to_host));
    }
};
...
struct session_data
{
    int id;
    ssh_channel channel;
    ssh_channel proxy_to_channel;
    int  authenticated;
    int  auth_attempts;
    int  erase_flag;

    session_data() : id(0), channel(NULL), proxy_to_channel(NULL), authenticated(0),
        ...
        int is_stderr, void *userdata)
{
    ...
    // Add 1) check if proxy_to_channel has been created
    if ( psdata->proxy_to_channel == NULL ) {
        printf("ERROR: my_channel_data_function_cb(%d) = proxy channel is NULL\n", my_Param->id);
        ssh_channel_write(channel, "ERROR: proxy channel is NULL", sizeof("ERROR: proxy channel is NULL"));
        return nbytes;
    } else {
        if ( !ssh_channel_is_open(psdata->proxy_to_channel) ) {
            int rc = ssh_channel_open_session(psdata->proxy_to_channel);
            if (rc == SSH_ERROR) {
                printf("ERROR: my_channel_data_function_cb(%d) -> ssh_channel_open_session() Failed\n",
                        my_Param->id);
                ssh_channel_write(channel, "ERROR: proxy channel is not Open", sizeof("ERROR: proxy channel is not Open"));
                return nbytes;
            }
        }
        // Add 2) Forward data to proxy_to_channel
        ssh_channel_write(psdata->proxy_to_channel, (char*)data, nbytes);
    }

    // Add 3) Read proxy_to_channel response back
    char proxy_buf[1024];
    memset(proxy_buf, '\0', 1024);
    int proxy_bytes = ssh_channel_read(psdata->proxy_to_channel, proxy_buf, 1024, 0);
    if ( proxy_bytes > 0 ) {
        printf("INFO: my_proxy_channel_data_reply(%d) [%d] - %s\n", my_Param->id, proxy_bytes, proxy_buf);
        ssh_channel_write(channel, proxy_buf, proxy_bytes);
    }
...
void my_handle_session(struct global_data myParam, ssh_session session)
{
    ...
    // Trying connect to proxy_to_host
    // Add 1) Create a new proxy_session
    ssh_session proxy_session = ssh_new();
    if (proxy_session == NULL) {
    }

    // Add 2) Set porxy_session options
    rc = ssh_options_set(proxy_session, SSH_OPTIONS_HOST, myParam.proxy_to_host);
    rc = ssh_options_set(proxy_session, SSH_OPTIONS_USER, myParam.Login);
    rc = ssh_options_set(proxy_session, SSH_OPTIONS_PORT, &myParam.proxy_to_port);
    rc = ssh_options_set(proxy_session, SSH_OPTIONS_LOG_VERBOSITY, 0);

    // Add 3) Connect to proxy_session
    rc = ssh_connect(proxy_session);
    if ( rc == SSH_OK ) {
        printf("INFO: my_handle_session(%d)<%d> -> ssh_connect(%s@%s:%d) - Success\n",
                myParam.id, (int)iVec,
                myParam.Login, myParam.proxy_to_host, myParam.proxy_to_port);
    } else {
        printf("ERROR: my_handle_session(%d)<%d> -> ssh_connect(%s@%s:%d) - [%d - %s]\n",
                myParam.id, (int)iVec,
                myParam.Login, myParam.proxy_to_host, myParam.proxy_to_port,
                ssh_get_error_code(proxy_session), ssh_get_error(proxy_session));
        ssh_disconnect(proxy_session);
        ssh_free(proxy_session);
        ssh_event_free(my_event);
        return;
    }

    // Add 4) Proxy server Authentication
    rc = ssh_userauth_password(proxy_session, myParam.Login, myParam.PassWd);
    if ( rc == SSH_AUTH_SUCCESS ) {
        printf("INFO: my_handle_session(%d)<%d> -> ssh_userauth_password(%s@%s:%d) - Success\n",
                myParam.id, (int)iVec,
                myParam.Login, myParam.proxy_to_host, myParam.proxy_to_port);
    } else {
        printf("ERROR: my_handle_session(%d)<%d> -> ssh_userauth_password(%s@%s:%d) - [%d - %s]\n",
                myParam.id, (int)iVec,
                myParam.Login, myParam.proxy_to_host, myParam.proxy_to_port,
                ssh_get_error_code(proxy_session), ssh_get_error(proxy_session));
        ssh_disconnect(proxy_session);
    ...
    // Add 5) Creat proxy_session channel
    vsdata[iVec].proxy_to_channel = ssh_channel_new(proxy_session);
    if ( vsdata[iVec].proxy_to_channel == NULL ) {
        printf("ERROR: my_handle_session(%d)<%d> -> ssh_channel_new(%d) - %s\n",
                myParam.id, (int)iVec,
                ssh_get_error_code(proxy_session), ssh_get_error(proxy_session));
        ssh_disconnect(proxy_session);
        ssh_free(proxy_session);
    ...
    ssh_channel_free(vsdata[iVec].proxy_to_channel);
    ssh_disconnect(proxy_session);
    ssh_free(proxy_session);
    ssh_event_free(my_event);
...
void *my_session_thread_function(void *arg)
{
    ...
    memcpy(myParam.proxy_to_host, gParam.proxy_to_host, sizeof(gParam.proxy_to_host));
    myParam.proxy_to_port = gParam.proxy_to_port;
...
int main(int argc, char* argv[])
{
    ...
    if ( argc != 7 ) {
        std::cout << "Usage : " << argv[0] << " <uri> <port> <login> <id_rsa> <proxy_to_host> <proxy_to_port>" << std::endl;
        std::cout << "Example : " << std::endl;
        std::cout << "        " << argv[0] << " 10.0.0.191 4567 hyuan /home/hyuan/.ssh/id_rsa 10.0.0.191 1234" << std::endl;
        exit(-1);
    }
    ...
    memcpy(gParam.proxy_to_host, argv[5], strlen(argv[5]));
    gParam.proxy_to_port = std::stoi(argv[6]);
    ...
    rc = ssh_bind_options_set(sshbind, SSH_BIND_OPTIONS_LOG_VERBOSITY_STR, 0);
    ...
    printf("INFO: ssh_bind_listen() on %s:%s proxy forwarding to %s:%s\n", argv[1], argv[2],
            argv[5], argv[6]);
