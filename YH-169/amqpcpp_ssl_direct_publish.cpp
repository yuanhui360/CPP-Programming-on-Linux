...
int main (int argc, char** argv) {
    ...
    // Add hard code assign 3 certificaties and key file
    ...
    try {
        ...
        // Add Modify AMQP constructor to enable TLS/SSL socket
        AMQP amqp(conn_str, true, ssl_cacert, client_cert, client_key, false, false);

        AMQPExchange * ex = amqp.createExchange(ex_name);
        // short ex_param = AMQP_AUTODELETE | AMQP_DURABLE;
        short ex_param =  AMQP_DURABLE;
        ex->setParam(ex_param);
        ex->Declare(ex_name, "direct", ex_param);
