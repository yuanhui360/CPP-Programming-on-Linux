...
int main(int argc, char * argv[])
{
    ...
    .onSuccess([&channel](const std::string &name, uint32_t messageCount, uint32_t consumerCount) {
            ...
            // Add logic to fetch messages using get method

            for ( uint32_t i = 0; i < messageCount; i++ ) {
                 channel.get(name, AMQP::noack)
                 .onSuccess([](const AMQP::Message &message, uint64_t deliveryTag, bool redelivered) {
                     ...
                     std::cout << "INFO: message received (" << deliveryTag  << "-"
                               << message.bodySize() << ") " << message_buf << std::endl;
                 }).onEmpty([]() {
                     std::cout << "INFO: channel.get().onEmpty() - Queue is EMpty" << std::endl;
                 })
                 .onFinalize([](){
                     std::cout << "INFO: channel.get().onFinalize() - Started" << std::endl;
                 })
                 .onError([](const char *message) {
                     std::cout << "ERROR: channel.get().onError() - " << message << std::endl;
                 });
            }
